...
int main(int argc, char* argv[])
{
    ...
    for (const auto& path : chd_files) {
        ...
        auto sys_all = det_all.system;
        auto sys_backup = det_backup.system;
        ...
        if (sys_all != sys_backup) {
            ...
            bool sector0_platform = (sys_all == chdlite::CdSystem::ThreeDO ||
                                     sys_all == chdlite::CdSystem::MegaCD ||
                                     sys_all == chdlite::CdSystem::Saturn);
            if (sector0_platform && sys_backup == chdlite::CdSystem::GenericCD)
                note = "  (backup->GenericCD expected)";
            else if (sys_all == chdlite::CdSystem::Dreamcast)
        ...
        std::printf("%-12s %-50s  %-6s  all=%-16s (%.1fms)  backup=%-16s (%.1fms)%s\n",
            parent.c_str(), name.c_str(), type_str,
            chdlite::system_name(sys_all), ms_all,
            chdlite::system_name(sys_backup), ms_backup,
            note);
